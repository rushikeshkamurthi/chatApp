...
const SelectedContact = ({item})=>{
  ...
  return(<StyledImageWrapper>
    <StyledImage source={{uri: item?.picture ? item?.picture : 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png' }}/>
    </StyledImageWrapper>)
}
...
const Chat = ({item}) => {
    ...
    font-weight:bold;
    `;
    const LastMsgTime = styled.Text`
    font-size: 10px;
    font-weight:400;    
    `;
    const LastMsgText = styled.Text`
    font-size: 12px;
    font-weight:400;    
    `;
    const FloatingActionButton = styled.TouchableOpacity`
...
return(
    <Card>
        <UserInfo>
            ...
            <TextSection><UserInfoText>
            <UserName>{item.name}</UserName>
            </UserInfoText><LastMsgText>{item.status}</LastMsgText></TextSection>
        </UserInfo>
    </Card>
)
}
  return (
    <View>
    <FlatList
    data ={data.allFriends}
    keyExtractor={item=>item.id}
    renderItem={Chat}
    ></FlatList>
    <FloatingActionButton onPress={()=>navigation.navigate('NewChat')}>
        <AddSymbol>Done</AddSymbol>
    </FloatingActionButton>
    </View>
  )
}

export default SelectMultipleContact;